pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'karthinachimuthu'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/Karthi-nachimuthu/SonarQube-Project-Kastro.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Karthi -Dsonar.projectKey=KarthiKey -Dsonar.java.binaries=target"
                }
            }
        }
